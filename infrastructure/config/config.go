...
func extrairDados(descricao string) (Dados, error) {
	...
	// Expressões regulares para extrair os valores do formato TNS
	serviceNamePattern := regexp.MustCompile(`service_name=([^)]+)`)
	...
	if len(serviceNameMatch) < 2 || len(portMatch) < 2 || len(hostMatch) < 2 {
		return dados, fmt.Errorf("não foi possível extrair todos os dados da string de conexão")
	}
